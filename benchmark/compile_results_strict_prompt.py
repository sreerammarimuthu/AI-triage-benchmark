"""
compile_results_strict_prompt.py — Generates results for the strict prompt variant.
...
# ── Paths ──────────────────────────────────────────────────────────────────
CKPT_V2   = "benchmark/output_strict_prompt/results/checkpoint.json"
OUT_PATH  = "benchmark/output_strict_prompt/reports/results.xlsx"
...
def _sheet_summary(wb, all_records, models_present):
    ...
    _hdr_cell(ws, 1, 1, "BENCHMARK (STRICT PROMPT) — MULTI-MODEL RESULTS SUMMARY",
...
def _sheet_model_io(wb, all_records, models_present):
    """Full I/O trace: what each model saw, what it said, and what each judge evaluated."""
    import pandas as pd

    ws = wb.create_sheet("Model_IO_Trace")

    base_dir = os.path.dirname(os.path.abspath(__file__))
    df_v2  = pd.read_csv(os.path.join(base_dir, "data", "vignettes.csv"))

    # Strict prompt variant: open-source models only — ChatGPT Health not included.
    input_oss = {str(row["case_id"]): str(row["input_prompt"]) for _, row in df_v2.iterrows()}

    def _swap_prefix(case_id):
        """After the E/F case_id swap fix in the checkpoint, open-source entries with
        case_id=E* were originally run on F* content (and vice versa). Swap the prefix
        to retrieve the actual input_prompt that was sent to the model."""
        if case_id.startswith("E"):
            return "F" + case_id[1:]
        elif case_id.startswith("F"):
            return "E" + case_id[1:]
        return case_id  # MH/NH — unaffected

    # ── Title + column group labels ──────────────────────────────────────────
    _hdr_cell(ws, 1, 1, "Model I/O Trace — What Each Model Saw, What It Said, and What Each Judge Evaluated",
              bg=C_HDR_BG, bold=True)
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=12)

    group_spans = [
        (1, 5, "CASE",             C_HDR_BG,  C_HDR_FG),
        (6, 6, "MODEL INPUT",      "1565C0",   "FFFFFF"),
        (7, 7, "MODEL OUTPUT",     "1565C0",   "FFFFFF"),
        (8, 9, "COHERENCE JUDGE",  "4527A0",   "FFFFFF"),
        (10,12,"FAITHFULNESS JUDGE","B71C1C",  "FFFFFF"),
    ]
    for c_start, c_end, label, bg, fg in group_spans:
        _hdr_cell(ws, 2, c_start, label, bg=bg, fg=fg)
        if c_end > c_start:
            ws.merge_cells(start_row=2, start_column=c_start, end_row=2, end_column=c_end)

    col_hdrs = [
        "Case ID", "Model", "Prompt Type", "Gold", "Predicted",
        "Input Prompt (full text model received)",
        "Reasoning / Explanation (extracted from response)",
        "Coherence Input: reasoning vs predicted triage",
        "Coherence Verdict",
        "Faithfulness Input: patient msg vs reasoning",
    ...
    recs_sorted = sorted(
        ...
        key=lambda r: (r.get("case_id", ""),
                       MODEL_IDS.index(r.get("model", "")) if r.get("model", "") in MODEL_IDS else 99)
    )

    for i, r in enumerate(recs_sorted, 4):
        halluc    = r.get("hallucination") or {}
        case_id   = r.get("case_id", "")
        model     = r.get("model", "")
        predicted = r.get("triage_level") or "—"
        reasoning = r.get("reasoning") or ""

        # Use swapped case_id to retrieve the actual prompt used before the E/F relabelling fix
        input_prompt = input_oss.get(_swap_prefix(case_id), "")

        coh_input   = f"Reasoning:\n{reasoning}\n\nPredicted triage: {predicted}"
        faith_input = f"Patient message:\n{input_prompt}"

        coh_pass   = halluc.get("coherence_pass")
        faith_pass = halluc.get("faithfulness_pass")
        faith_verd = halluc.get("faithfulness_verdict") or "—"
        ...
        vals = [
            ...
            reasoning,
            coh_input,
            "YES" if coh_pass is True else ("NO" if coh_pass is False else "—"),
            faith_input,
            faith_verd,
            halluc.get("faithfulness_fabricated") or "—",
        ]

        row_bg = MODEL_COLORS.get(model)
        for j, v in enumerate(vals, 1):
            if j == 9:   # coherence verdict
                bg = C_PASS if v == "YES" else (C_FAIL if v == "NO" else C_NULL)
                c = ws.cell(row=i, column=j, value=v)
                c.fill = _fill(bg); c.font = _font(); c.alignment = _align(); c.border = _thin_border()
            elif j == 11:  # faithfulness verdict
                bg = C_PASS if v == "NO" else (C_FAIL if v == "YES" else C_NULL)
                c = ws.cell(row=i, column=j, value=v)
                c.fill = _fill(bg); c.font = _font(); c.alignment = _align(); c.border = _thin_border()
            else:
                halign = "left" if j in (6, 7, 8, 10, 12) else "center"
                bg_use = row_bg if j == 2 else None
                c = ws.cell(row=i, column=j, value=v)
                if bg_use: c.fill = _fill(bg_use)
                c.font = _font(); c.alignment = _align(h=halign, wrap=True); c.border = _thin_border()

    # Notes
    note_row = 4 + len(recs_sorted) + 1
    notes = [
        "• Model Input (col 6): all models received text from vignettes.csv (input_prompt). ChatGPT Health is not included in the strict prompt variant.",
        "• Coherence judge (Llama 3.1 8B) evaluated: does the reasoning logically support the predicted triage level? VERDICT: YES = coherent, NO = incoherent",
        "• Faithfulness judge (Llama 3.1 8B) evaluated: does the reasoning fabricate patient-specific facts absent from the patient message? VERDICT: YES = fabrication, NO = faithful",
        "• Coherence judge input = col 8 (reasoning + predicted triage). Faithfulness judge input = col 10 (patient message + reasoning)",
    ]
    for k, note in enumerate(notes, note_row):
        ws.cell(row=k, column=1, value=note).font = _font(size=9)
        ws.merge_cells(start_row=k, start_column=1, end_row=k, end_column=12)

    _set_col_widths(ws, [10, 16, 14, 8, 10, 70, 70, 70, 12, 70, 12, 50])
    ws.freeze_panes = "A4"
...
def compile_results():
    ...
    _sheet_model_io(wb, all_records, models_present)
    for m in models_present:
